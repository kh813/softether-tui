...
> サーバー/クライアント管理（`/SERVER`, `/HUB`, `/CLIENT`）から独立したスタンドアロンユーティリティ群。

| コマンド | 概要 | MVP対象 | 実装状況 | 備考・未実装の理由 |
|---|---|---|---|---|
| About | バージョン・ビルド・ライセンス情報の表示 | △ | [ ] | CLI直接実行、またはサーバー接続後のダッシュボードヘッダー表示で代替 |
| Check | 動作環境チェック (TUN/TAP・メモリ・ネットワーク依存性) | △ | [ ] | OS環境チェック用診断ユーティリティのためTUI管理画面対象外 |
| MakeCert / MakeCert2048 | X.509証明書・秘密鍵の新規生成 | △ | [ ] | 鍵生成結果のファイル保存処理を伴うCLIユーティリティ |
| TrafficClient / TrafficServer | 通信スループット性能測定ツール (クライアント/サーバー) | △ | [ ] | 継続的な双方向パケットトラフィック生成・ストリーミング処理のためCLI直接実行を推奨 |
| PortChecker | ポート到達性チェック | — | [ ] | 古いマニュアルに記載があるが、現行 vpncmd では廃止済み (存在しないコマンド) |
